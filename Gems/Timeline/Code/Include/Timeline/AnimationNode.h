...
namespace Timeline
{
    ...
    public:
        ...
        struct SParamInfo
        {
            ...
            AnimValueType valueType; // defines type of track to use for animating this parameter
        ...
        virtual IAnimationTrack* GetTrackForParameter(const CAnimParamType& paramType) const = 0;

        // Return the i-th track assigned to the specified parameter in case of multiple tracks.
        virtual IAnimationTrack* GetTrackForParameter(const CAnimParamType& paramType, uint32 index) const = 0;
        ...
        virtual IAnimationTrack* CreateTrack(const CAnimParamType& paramType) = 0;

        // Initializes track default values after de-serialization / user creation. Only called in editor.
        virtual void InitializeTrackDefaultValue(IAnimationTrack* pTrack, const CAnimParamType& paramType) = 0;

        // Assign animation track to parameter.
        // if track parameter is NULL track with parameter id param will be removed.
        virtual void SetTrack(const CAnimParamType& paramType, IAnimationTrack* track) = 0;
